return function(context)
	...
	local function f3_body(real_dt)
			...
			-- force_smooth already folds in max_fid (line 226).
			if force_smooth then
				sm_alpha = 1
			end
			...
			for k = #arr, 1, -1 do
				...
				if distance_sq > c7_sq or always_process or d.pc_mode or is_cursed_red or d.free_active or d.collision_active or d.angular_active then
					...
					if pc == "pin" or pc == "manual" then
						...
						-- about a second to settle and visibly trailed the cursor through a
	...
	-- is and Mouse.Target still finds it, so dragging the core around while it is
	-- hidden keeps working exactly as it already did while Disabled.
