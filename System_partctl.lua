return function(context, x7)
	-- context.v9 (the legacy Mouse) is deliberately not taken. Picking held parts goes
	-- through an Include raycast now: Mouse.Target returns whatever is nearest the
	-- cursor whether it is held or not, which is why a held part behind anything else
	-- could not be clicked, and why the click landed in the box-select branch instead.
	...
	-- How far the pointer has to travel before a press becomes a drag. Without a
	-- threshold every click was a drag and every drag latched an override, so merely
	-- selecting a part pinned it -- including with the panel's own mode row reading
	-- "Normal (No Override)".
	local DRAG_PX = 6
	...
	-- is going to place by hand. Box-select has always had the same exposure and now
	-- shares the cap.
	...
	local function pc_clear_highlights()
		...
		x6.pc_drag_size = nil
		-- The pending press too, or a click whose gesture is cancelled by a panel
		-- teardown leaves a part armed to start a drag on the next mouse move.
		x6.pc_press = nil
		x6.pc_box_start = nil
		pc_changed()
	end
	...
	-- Bulk selection. Box-select covers this on desktop and does not exist on the
	-- mobile tree at all, so these are also the only way a touch user selects more
	-- than one part. Walking active_array rather than the weak hash keeps the order
	-- stable and skips entries the sweep has not reaped yet.
			...
			if d then
				...
				if mode == nil then
					...
					elseif mode == "shape" then
						...
						-- used to increment anyway -- so pressing Assign Shape twice, or
	...
	local function pc_latch_drag()
		...
		for part, _ in pairs(x6.pc_selected) do
			...
			if d and d.pc_target then
				...
				-- part.Position, and a dragged part trails the cursor by a frame, so every
	...
	-- Picking. This tree used Mouse.Target, which returns whatever is nearest the
	-- cursor whether it is held or not -- so a held part behind any other part could
	-- not be clicked at all, and the click fell through to the box-select branch,
	-- which *clears the selection*. An Include filter over the held parts answers the
	-- only question this needs to ask, and it is what the mobile tree already did.
	--
	-- active_array rather than pairs(x6.a): the dense array makes the candidate list
	-- a straight copy, and it is what every other bulk walk in the tree uses.
	...
	local function pc_pick(screen_pos)
		...
		-- keep a released part clickable.
	...
	-- sphere at the distance latched when the press began -- no way to push it away or
	-- pull it closer, which is the one thing a placement tool has to do -- and it
	-- ignored the world entirely, so putting a part on the floor was guesswork.
	--
	...
	local function pc_drag_point(screen_pos, distance, anchor_size)
		...
		if x1.PartCtlSurfaceSnap ~= false then
			...
			-- the cursor whenever a drag crosses it.
	...
	return function()
		...
		-- Everything a drag needs, in one place, because a press does not know yet
		-- whether it is going to become one. Nothing here runs on a click.
		local function begin_drag(anchor)
			...
			for part, _ in pairs(x6.pc_selected) do
				...
				if d then
					...
					-- the part sat exactly where the shape wanted it while the cursor
		...
		table.insert(
			...
			v1.InputBegan:Connect(function(input, processed)
				...
				if input.UserInputType == Enum.UserInputType.MouseButton1 then
					local screen = v1:GetMouseLocation()
					...
					if target and x6.a and x6.a[target] then
						if x6.pc_selected[target] and shift_held then
							pc_deselect(target)
							return
						end
						...
						if not shift_held then
							...
							-- the latch unconditionally -- so a plain click pinned the
							-- part, with the panel's own mode row reading "Normal (No
							-- Override)". begin_drag runs from InputChanged, once the
							-- pointer has actually moved DRAG_PX.
		...
		table.insert(
			...
			v1.InputChanged:Connect(function(input, processed)
				...
				if x6.pc_dragging then
					move_drag(v1:GetMouseLocation())
					return
				end
				-- The threshold. A press that never travels this far stays a selection,
				-- which is what makes clicking a part free of consequences.
				local press = x6.pc_press
				if press then
					local cur = v1:GetMouseLocation()
					local dx, dy = cur.X - press.screen.X, cur.Y - press.screen.Y
		...
		table.insert(
			...
			v1.InputEnded:Connect(function(input)
				if input.UserInputType == Enum.UserInputType.MouseButton1 then
					-- A press that never crossed the threshold is a click, and a click is
					-- a selection: the latch only runs for a drag that actually happened.
					x6.pc_press = nil
					if x6.pc_dragging then
						x6.pc_dragging = false
						x6.pc_drag_target = nil
					...
					if x6.pc_box_start then
						...
						local arr = x6.active_array
						if cam and arr then
							-- The dense array, not pairs(x6.a): the weak hash still holds
							-- parts the sweep has not reaped, and its order is arbitrary.
							local n = pc_count()
							for i = 1, #arr do
								if n >= MAX_SELECT then
									break
